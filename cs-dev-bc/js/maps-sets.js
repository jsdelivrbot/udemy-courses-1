class MessageBoard {
    ...
    /*
    In your constructor method, you should assign two properties for 
    each object created from the MessageBoard class. 
    ...
    */
    ...
    The function should add a key and value to the messages map with a key of whatever the value of this.id 
    is and a value of whatever the string is that is passed to the function. 
    ...
    */
    ...
    findMessageById(key) {
        return this.messages.get(key) ? this.messages.get(key) : undefined;
    }

    /*
    Add a method called findMessageByValue which accepts a string and returns the message
    in the messages map with the same value as the string passed to the function. 
...
function countPairs(array, number) {
    const cache = new Set(array);
    let count = 0;

    for (let val of array) {
        cache.delete(val)
        if (cache.has(number - val)) {
            count++;
        }
    }
    return count;
